...
/** Simple editor color swatch selector used by save/folder dialogs. */
public final class ColorPaletteWidget extends AbstractWidget {
    ...
    protected void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        for (int color : DEFAULT_COLORS) {
            ...
            int border = sameRgb(color, selectedColor) ? 0xFFFFFFFF : 0xFF101318;
            graphics.outline(x - 1, getY() - 1, swatchRight - x + 2, height + 2, border);
            x += swatchWidth + gap;
        }
    }
    ...
    public void onClick(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (index < 0 || index >= DEFAULT_COLORS.length) return;
        if ((int) local % stride >= swatchWidth) return;
