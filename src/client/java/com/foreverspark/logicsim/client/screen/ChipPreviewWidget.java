...
public final class ChipPreviewWidget extends AbstractWidget {
    ...
    public void setBounds(int x, int y, int width, int height) {
        setX(x); setY(y); this.width = Math.max(80, width); this.height = Math.max(80, height);
    }
    public void setPortCounts(int inputs, int outputs) { inputCount = Math.max(0, inputs); outputCount = Math.max(0, outputs); bodyHeight = Math.max(bodyHeight, minimumHeight()); }
    ...
    public void autoFit() { bodyWidth = minimumWidth(); bodyHeight = minimumHeight(); }
    ...
    protected void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        graphics.text(font(), shown, bodyX + (bodyScreenWidth - font().width(shown)) / 2, bodyY + Math.max(8, (int) Math.round(10 * renderScale)), 0xFFF1F4F7, false);
        ...
        graphics.text(font(), "Drag any edge / corner to resize", getX() + 8, getY() + height - 14, 0xFF65717E, false);
    }

    @Override public void onClick(MouseButtonEvent event, boolean doubleClick) { if (event.button() == 0 && active && visible) { dragMode = resizeModeAt(event.x(), event.y()); dragScale = Math.max(0.001, renderScale); } }
    ...
    @Override public void onRelease(MouseButtonEvent event) { if (event.button() == 0) dragMode = DragMode.NONE; }
    ...
    private DragMode resizeModeAt(double mouseX, double mouseY) {
        ...
        if (left) return DragMode.LEFT; if (right) return DragMode.RIGHT; if (top) return DragMode.TOP; if (bottom) return DragMode.BOTTOM; return DragMode.NONE;
    }

    private double minimumWidth() { return snap(clamp(font().width(normalizedName().isBlank() ? "CHIP" : normalizedName()) + 30.0, MIN_WIDTH, MAX_WIDTH)); }
    private double minimumHeight() { int count = Math.max(inputCount, outputCount); return snap(clamp(count <= 1 ? 54.0 : 36.0 + (count - 1) * MIN_PIN_GAP, MIN_HEIGHT, MAX_HEIGHT)); }
    private double automaticPinGap(double height) { int count = Math.max(inputCount, outputCount); if (count <= 1) return 18.0; return snap(clamp(Math.max(MIN_PIN_GAP, (height - 36.0) / (count - 1)), MIN_PIN_GAP, MAX_PIN_GAP)); }
    private String normalizedName() { String value = nameSupplier.get(); return value == null ? "" : value.trim(); }
    private String fitText(String value, int maxPixels) { if (font().width(value) <= maxPixels) return value; int max = Math.max(1, value.length()); while (max > 1 && font().width(value.substring(0, max - 1) + "…") > maxPixels) max--; return value.substring(0, Math.max(0, max - 1)) + "…"; }
    ...
    private enum DragMode {
        ...
        int xSign() { return xSign; } int ySign() { return ySign; }
        boolean horizontal() { return xSign != 0; } boolean vertical() { return ySign != 0; }
        boolean touchesLeft() { return xSign < 0; } boolean touchesRight() { return xSign > 0; }
        boolean touchesTop() { return ySign < 0; } boolean touchesBottom() { return ySign > 0; }
