...
public final class SourceConfigScreen extends Screen {
    ...
    private void apply() {
        try {
            ...
            error = mode == Mode.CLOCK ? "Enter a number, for example 20, 50, or 100" : "Enter a whole number from 0 to 100";
    ...
    private long parseFrequency() {
        ...
        if (exact < 1.0 || exact > ClockPlacementState.MAX_FREQUENCY_HZ) {
            throw new IllegalArgumentException("Frequency must be from 1 Hz to 100 MHz");
        }
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        if (mode == Mode.CLOCK) {
            ...
            catch (RuntimeException ignored) { resolved = "Range: 1 Hz .. 100 MHz"; }
